...
当 Agent（或人类）新增了基建（Registry、Pipeline、System 抽象、公共 Mod），必须同步产出文档。但文档不能乱放——遵循以下规则。

### 5.1 文档放在哪

| 产出类型 | 文档位置 | 编号规则 |
|---------|---------|---------|
| 新的架构概念、子系统设计 | `docs/developer-guide/NN_topic.md` | 取当前最大编号 +1，不重排 |
| 新的编码规则、流程约束 | `docs/conventions/` | 提议后由用户确认是否加入 |
| Mod 使用说明 | Mod 目录内 `README.md` | 仅限该 Mod 内部 |

**禁止**：在仓库根目录、`src/` 内、或任意随机位置创建 `.md` 文件。

### 5.2 架构文档必须包含什么

每篇基建文档必须覆盖以下五项，缺一不可：

1. **一句话定位**：这个基建解决什么问题，谁来用
2. **公开 API**：接口/方法签名、参数含义、调用时机
3. **最佳实践**：用"正确做法 vs 错误做法"的对比表呈现
4. **示例代码**：站在调用方视角，展示最小可运行用法
5. **集成点**：挂靠哪个 Registry/Pipeline/SystemGroup phase

### 5.3 示例代码规范

*   语言标签必须标注（` ```csharp `）
*   只展示调用方视角（"怎么用"），不展示内部实现
*   占位符用 `/* ... */`，不用 `...` 省略号
*   去掉与主题无关的 `using`
*   每个示例必须能让读者复制后直接写出正确调用

**正例**（调用方视角）：

```csharp
// 在 Mod 的 OnLoad 中注册一个可选 System
context.SystemFactoryRegistry.Register(
    "MyFeatureSystem",
    world => new MyFeatureSystem(world, context.AttributeRegistry),
...
```csharp
// SystemFactoryRegistry 内部实现
public void Register(string key, Func<World, BaseSystem<World, float>> factory, SystemGroup group)
{
    _factories[key] = (factory, group);
}
```

### 5.4 文档格式检查清单

提交文档前确认（完整规范见 `docs/developer-guide/00_documentation_standards.md`）：

*   [ ] 文件名 `NN_snake_case_topic.md`，编号正确
*   [ ] 不含内部术语（阶段编号、工单号、AI 署名）
*   [ ] 不含推测或未实现功能的描述
*   [ ] `docs/developer-guide/README.md` 或 `docs/conventions/README.md` 目录已同步
*   [ ] 五项必含内容齐全（定位、API、最佳实践、示例、集成点）
